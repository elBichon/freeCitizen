...
default;
    ...
    if ($query->rowCount()>0)
    {
        ...
        while ($data = $query->fetch())
        {
            ...
            if($data['mp_lu'] == 0)
            {
            echo'<td><img src="images/message_non_lu.png" alt="Non lu" /></td>';
            }
            else //sinon une autre icone
            {
            echo'<td><img src="images/message.png" alt="Déja lu" /></td>';
            }
            echo'<td id="mp_titre">
            <a href="messagesprives.php?action=consulter&amp;id='.$data['mp_id'].'">
            '.stripslashes(htmlspecialchars($data['mp_titre'])).'</a></td>
            <td id="mp_expediteur">
            <a href="profil.php?action=consulter&amp;m='.$data['membre_id'].'">
